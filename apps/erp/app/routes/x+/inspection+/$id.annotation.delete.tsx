...
import { deleteBalloonAnnotations } from "~/modules/quality";
...
export async function action({ request, params }: ActionFunctionArgs) {
  ...
  const result = await deleteBalloonAnnotations(client, {
  ...
  if (result.error) {
    return data(
      {
        success: false,
        message: getErrorMessage(result.error, "Failed to delete annotations")
      },
      { status: 400 }
    );
  }
