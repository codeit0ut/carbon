...
export default function BalloonDocumentEditor({
  diagramId,
  name,
  content,
  anchors,
    ...
    (x: number, y: number) => {
      ...
      if (dragKind === "anchor") {
        ...
        const anchorY = Math.max(
          0,
          Math.min(100, localY + clippedLocalHeight / 2)
        );
    ...
    (e: unknown) => {
      ...
      if (dragKind === "anchorResize" && drag && anchorResizeRef.current) {
        ...
        const centerX = Math.max(
          0,
          Math.min(100, resizedRect.x + resizedRect.width / 2)
        );
        const centerY = Math.max(
          0,
          Math.min(100, resizedRect.y + resizedRect.height / 2)
        );
        setFeatureRows((prev) =>
          prev.map((row) =>
  ...
  const handlePdfUpload = useCallback(
    async (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      const storagePath = `${companyId}/balloonDocument/${diagramId}/${nanoid()}.pdf`;
